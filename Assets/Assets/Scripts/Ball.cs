...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] private GameObject powerAzul;
    [SerializeField] private GameObject powerVerde;
    void Start()
    {
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            {
                rb.velocity = new Vector2(rb.velocity.x*velocityMultiplier,rb.velocity.y);
            }
            if(Mathf.Abs(rb.velocity.y)<= velocidadMaxima)
            {
                rb.velocity = new Vector2(rb.velocity.x, rb.velocity.y * velocityMultiplier);
            }
        ...
        } else 
        {
            ...
            if(random2 < 0.2f)
            {
                Instantiate(powerAzul, collision.transform.position, Quaternion.identity);
            }
            ...
            if(random3 < 0.5f)
            {
                Instantiate(powerVerde, collision.transform.position, Quaternion.identity);
